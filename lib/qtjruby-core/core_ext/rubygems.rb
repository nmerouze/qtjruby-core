...
module Gem
  class SourceIndex
    ...
    def add_spec(gem_spec)
      @gems[gem_spec.full_name] = gem_spec unless @gems[gem_spec.full_name].is_a?(Gem::Specification) && @gems[gem_spec.full_name].installation_path == File.join(defined?(Qt::JRuby) && Qt::JRuby.respond_to?(:root) ? Qt::JRuby.root : Dir.pwd,"gems")
    end
  end
  ...
  class Specification
    ...
    def sort_obj
      [@name, installation_path == File.join(defined?(Qt::JRuby) && Qt::JRuby.respond_to?(:root) ? Qt::JRuby.root : Dir.pwd,"gems") ? 1 : -1, @version.to_ints, @new_platform == Gem::Platform::RUBY ? -1 : 1]
    end
  end
end
